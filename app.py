...
if "manual_route_mode" not in st.session_state:
    st.session_state.manual_route_mode = False
...
with right:
    if mode == "Marcar presas":
        ...
        st.divider()
        st.caption("Zona segura: borrar presas elimina tambien las rutas de esta foto.")
        delete_holds_confirmation = st.text_input(
            "Para borrar escribe BORRAR",
            value="",
            key=f"delete-holds-{image_key}",
        )
        if st.button(
            "Borrar presas de esta imagen",
            disabled=not holds or delete_holds_confirmation != "BORRAR",
        ...
        st.subheader("Ruta")
        st.subheader("Ruta manual")
        st.caption("Inicia una ruta manual y luego toca las presas en el orden que quieres escalarlas.")
        c_manual_1, c_manual_2 = st.columns(2)
        if c_manual_1.button("Nueva ruta manual", use_container_width=True):
            st.session_state.manual_route_mode = True
            st.session_state.active_route_name = "Ruta manual"
            st.session_state.active_route_holds = []
            st.rerun()
        if c_manual_2.button(
            "Terminar edicion",
            disabled=not st.session_state.manual_route_mode,
            use_container_width=True,
        ):
            st.session_state.manual_route_mode = False
            st.rerun()
        ...
        show_unselected_holds = st.toggle(
            ...
            value=st.session_state.manual_route_mode or not st.session_state.active_route_holds,
        ...
        if st.session_state.manual_route_mode:
            st.info("Edicion manual activa: toca presas para agregarlas o quitarlas de la ruta.")
        else:
            st.caption("Para editar a mano, usa 'Nueva ruta manual' o activa 'Mostrar presas no usadas'.")
        c1, c2 = st.columns(2)
        if c1.button("Quitar ultima", disabled=not st.session_state.active_route_holds):
            st.session_state.active_route_holds.pop()
            st.rerun()
        clear_route_confirmation = st.checkbox(
            "Confirmar limpiar ruta actual",
            disabled=not st.session_state.active_route_holds,
        )
        if c2.button(
            "Limpiar ruta",
            disabled=not st.session_state.active_route_holds or not clear_route_confirmation,
        ):
            st.session_state.active_route_holds = []
            st.rerun()
